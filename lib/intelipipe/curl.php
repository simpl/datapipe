  ...
  */
  ...
  'defaults'=> array (
    ...
    'all' => array (
      ...
      'connect-timeout'           =>  30,       # 0 to wait indefinitely
      'insecure'                  => true,
      'progress-bar'              => true,
    ),
  ...
  'options' => array (
    ...
    'http' => array (
      ...
      'location'                  =>  'b',      # follow Location headers (recursively) - unlimited unless CURLOPT_MAXREDIRS is set
      'max-redirs'                =>  array (
                                        'i_ma',
                                        20,
                                      ),
...
function  curl ($opts, $pipe, $cmd = __FUNCTION__, $default_protocol = null) {
  ...
  if (!$protocol) {
    ...
    elseif  ($substr_7 == 'imap://')    $protocol = 'imap';
    elseif  ($substr_8 == 'imaps://')   $protocol = 'imaps';
    elseif  ($substr_7 == 'smtp://')    $protocol = 'smtp';
    elseif  ($substr_8 == 'smtps://')   $protocol = 'smtps';
  ...
  $save_to_files = array (
    ...
    #'response_heads' => array (null,                  $response_heads),
    'response_head'  => array ($response_head_file,   $response_head, true),
...
function  curl_build_request ($prefix, $protocol, $curlopts, $cmd, $internal, $debug) {

  $request_cmd = array ('curl');
  ...
  if (curl_set_curlopts ($prefix, $request_cmd, $defaults['all'], $curlopts, $cmd, $debug) === false)
    return  false;
    
  if (curl_set_curlopts ($prefix, $request_cmd, $defaults[$protocol], $curlopts, $cmd, $debug) === false)
    return  false;
  
  if (curl_set_curlopts ($prefix, $request_cmd, $curlopts, null, $cmd, $debug, $options) === false)
    return  false;
  
  return  implode ('', $request_cmd);
}
  
  
function  curl_set_curlopts ($prefix, &$request_cmd, $curlopts, $user_curlopts, $cmd, $debug, $options = null) {


  # TODO: check the option is valid

  foreach ($curlopts as $name => $value) {
  
    if ($name == 'headers') {
    
      foreach ($value as $header) {
      
        $request_cmd[] =  " --header '$header'";
        
        if ($debug) {
        
          debug_echo ($cmd, "setting curlopt : header = $header");
        }
      }
    
    } else {
  
      if ($user_curlopts && @isset ($user_curlopts[$name]))
        continue;
...
function  curl_check_saved_file ($opts, $pipe, $cmd = __FUNCTION__, $prefix = 'curl') {
  ...
  $use_saved_file = get_opt ($prefix, $opts, 'use_saved_file', true);
